...
var TransactionSystem = (function () {
    function TransactionSystem(systemConfig, DbConnection) {
        ...
        this.orderShipped$ = function (transaction) {
            return new rxjs_1.Observable(function (observer) {
                var shippedTransaction = __assign({}, transaction, { order: __assign({}, transaction.order, { status: "SHIPPED" }), timestamp: new Date(new Date(transaction.timestamp).getTime() + utils_1.generateRandomInt(1, 3) * 24 * 60 * 60 * 1000) });
        ...
        this.orderDelivered$ = function (transaction) {
            return new rxjs_1.Observable(function (observer) {
                var deliveredTransaction = __assign({}, transaction, { order: __assign({}, transaction.order, { status: "DELIVERED" }), timestamp: new Date(new Date(transaction.timestamp).getTime() + utils_1.generateRandomInt(2, 7) * 24 * 60 * 60 * 1000) });
        ...
        this.orderCancelled$ = function (transaction) {
            return new rxjs_1.Observable(function (observer) {
                var cancelledTransaction = __assign({}, transaction, { order: __assign({}, transaction.order, { status: "CANCELLED" }), timestamp: new Date(new Date(transaction.timestamp).getTime() + utils_1.generateRandomInt(2, 7) * 24 * 60 * 60 * 1000) });
        ...
        this.orderReturned$ = function (transaction) {
            return new rxjs_1.Observable(function (observer) {
                var returnedTransaction = __assign({}, transaction, { order: __assign({}, transaction.order, { status: "RETURNED" }), timestamp: new Date(new Date(transaction.timestamp).getTime() + utils_1.generateRandomInt(2, 7) * 24 * 60 * 60 * 1000) });
