...
var TransactionSystem = (function () {
    function TransactionSystem(systemConfig, DbConnection) {
        ...
        this.getOrderDate = function () {
            var dayFactor = eval(moment(_this.currentSysTime).format("DDD"));
            if (moment(_this.currentSysTime).format("YY") === "17")
                dayFactor += 366;
            if (_this.currentSysTime.getTime() < Date.now())
                _this.currentSysTime = new Date(_this.currentSysTime.getTime() + (utils_1.generateRandomInt(60000, 300000) / dayFactor));
            else
